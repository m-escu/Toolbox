...
NetSpeedTest() {
    ...
    psLines.Push('try {')
    psLines.Push('  $meta = Invoke-RestMethod "https://speed.cloudflare.com/meta" -TimeoutSec 8 -UserAgent $ua')
    psLines.Push('  Write-Host ("ISP:      {0}" -f $meta.asOrganization)')
    psLines.Push('  Write-Host ("Your IP:  {0}" -f $meta.clientIp)')
    psLines.Push('} catch { Write-Host "Connection info unavailable (endpoint blocked)" -ForegroundColor DarkYellow }')
    psLines.Push('')
